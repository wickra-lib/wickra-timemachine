...
This makes the pairing explicit:
  ...
  * Every `wickra_*` symbol `src/wickra_timemachine.c` calls must be declared in
    `bindings/c/include/wickra_timemachine.h`. A call to a symbol that is not
...
downloads a prebuilt `wickra-timemachine-c-<triple>.tar.gz` for the version in
...
HEADER = "bindings/c/include/wickra_timemachine.h"
