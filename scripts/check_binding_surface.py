...
make, so this reads `wickra_timemachine_<name>` out of
`bindings/c/include/wickra_timemachine.h` and checks each language's public surface
...
HEADER = os.path.join(ROOT, "bindings", "c", "include", "wickra_timemachine.h")

# Exports that are ABI plumbing rather than a promise to callers.
ABI_ONLY = {"free"}

EXPORT = re.compile(r"\bwickra_timemachine_([a-z0-9_]+)\s*\(")
...
# file only re-exports `TimeMachine` from the compiled module, so the methods
...
BINDINGS: dict[str, tuple[list[str], dict[str, str]]] = {
    "python": (
        ["bindings/python/src/lib.rs", "bindings/python/python/wickra_timemachine/__init__.py"],
        {
            "new": r"(?m)^\s*fn new\s*\(",
    ...
    "csharp": (
        ["bindings/csharp/WickraTimeMachine/TimeMachine.cs"],
        {
            "new": r"(?m)^\s*public TimeMachine\s*\(",
    ...
    "go": (
        ...
        {
            "new": r"(?m)^func New\s*\(",
            "command": r"(?m)^func \([^)]*\*TimeMachine\) Command\s*\(",
            "version": r"(?m)^func Version\s*\(",
        },
    ),
    "java": (
        ["bindings/java/src/main/java/org/wickra/timemachine/TimeMachine.java"],
        {
            "new": r"(?m)^\s*public TimeMachine\s*\(",
            "command": r"(?m)^\s*public [^\n]*\bcommand\s*\(",
            "version": r"(?m)^\s*public static [^\n]*\bversion\s*\(",
    ...
    "r": (
        ["bindings/r/R/timemachine.R", "bindings/r/NAMESPACE"],
        {
            "new": r"(?m)^wktimemachine_new\s*<-\s*function",
            "command": r"(?m)^wktimemachine_command\s*<-\s*function",
            "version": r"(?m)^wktimemachine_version\s*<-\s*function",
        },
    ),
    ...
    "cpp": (
        ["bindings/c/include/wickra_timemachine.hpp"],
        {
            "new": r"\bTimeMachine\s*\(\s*(?:const\s+)?std::string",
...
def main() -> int:
    ...
    if not exports:
        print(f"no wickra_timemachine_* exports found in {HEADER}", file=sys.stderr)
        return 1
