...
A follow-up release: the replay engine and its bindings are unchanged. It pins
...
- **Built on wickra-exchange 0.1.8.** The exact pin on `wickra-exchange-core`
  moves from =0.1.7 to =0.1.8, and every tracked lockfile follows. Nothing in
  exchange's Rust API changed between the two; 0.1.8 fixes its R package's
  WebAssembly build and its Maven Central step.

## [0.1.3] - 2026-09-23

A maintenance release: the replay engine and its bindings are unchanged. It
...
- **The family pins follow the owners' releases.** `wickra-backtest` =0.1.7 ->
  =0.1.8, `wickra-exchange-core` =0.1.6 -> =0.1.7 -- the exact pins this
...
- **Third-party dependencies refreshed.** `Cargo.lock` takes 73 crates to their
...
- **The Go install line names the published module.** The badge and `go get`
  ...
  `github.com/wickra-lib/wickra-timemachine/bindings/go`, which the Go proxy never
  serves; they now name the mirror `github.com/wickra-lib/wickra-timemachine-go`, as the
...
- **CI is green again.** The benchmark panicked before measuring anything:
  its opening book snapshot carried no `timestamp`, which the feed parser
  requires (`missing field timestamp`). The napi glue
  (`bindings/node/index.js`) was stale against the locked CLI, so the in-sync
  check failed on every Node job; it is regenerated. The Examples job ran
  `cargo run -p wickra-timemachine-example` against a crate that is not a
  workspace member, pointed `dotnet run` at a project directory that does
  not exist (`Seek` is the project), and the Node and C# examples depended
...
- **The exported R functions are documented.** `wktimemachine_new`, `wktimemachine_command`
  and `wktimemachine_version` carried roxygen comments but no generated `man/` pages,
...
[Unreleased]: https://github.com/wickra-lib/wickra-timemachine/compare/v0.1.4...HEAD
[0.1.4]: https://github.com/wickra-lib/wickra-timemachine/compare/v0.1.3...v0.1.4
[0.1.3]: https://github.com/wickra-lib/wickra-timemachine/compare/v0.1.2...v0.1.3
